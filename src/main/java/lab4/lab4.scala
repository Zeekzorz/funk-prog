...
object RTUSet {
  ...
  /**
    ...
    */
  def singletonSet(elem: Int): Set = mySet => mySet == elem
  ...
  def exists(s: Set, p: Int => Boolean): Boolean = {
    def iter(myInt: Int): Boolean = {
      if (myInt > bound) false
      else if (s(myInt) && p(myInt)) true
      else iter(myInt + 1)
    }
  ...
  def toString(s: Set): String = {
    val t = for (i <- -bound to bound if contains(s, i)) yield i
    t.mkString("{", ",", "}")
  }
